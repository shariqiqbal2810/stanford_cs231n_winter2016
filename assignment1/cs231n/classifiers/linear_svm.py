...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #############################################################################
  num_train = X.shape[0]
  scores = X.dot(W) # gives class scores for each data point in rows
  ...
  # loss.                                                                     #
  #############################################################################
  is_wrong = (margins > 0).astype('int')
  is_wrong[range(num_train), y] = -(is_wrong).sum(axis=1)
  dW += X.T.dot(is_wrong)
  dW /= num_train
  dW += reg * W
